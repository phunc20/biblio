...
# ╔═╡ 0d11a0fe-6ca8-493a-98b0-958c72a29b2e
md"""
**Déf.** Un intervalle de confiance de seuil ``\alpha`` pour un paramètre ``\theta`` est un intervalle _aléatoire_ ``I`` tel que ``P(\theta \in I) = 1-\alpha``. A part le nom de **seuil**, le nombre ``\alpha`` est aussi connu sous le nom de **niveau de signification**.
"""

# ╔═╡ fc5101d8-8001-11eb-3f18-3572043ced9c
md"""
En `R`, la valeur $u_{\alpha}$ t.q. $P(\lVert U \rVert > u_{\alpha}) = \alpha\,$
peut etre obtenue par la commande `qnorm(1-alpha/2)`, où
...
bruit = [54.8, 55.4, 57.7, 59.6, 60.1, 61.2, 62.0, 63.1, 63.5,
         64.2, 65.2, 65.4, 65.9, 66.0, 67.6, 68.1, 69.5, 70.6,
	     71.5, 73.4
]

# ╔═╡ 063b5774-ddb6-495f-85e0-5de08f88e6bb
OneSampleTTest(bruit)

# ╔═╡ f8782bf9-9992-4fa8-bbd8-652e30fe6448
#confint(OneSampleTTest(); alpha=0.05, tail=:both)
