...
# ╔═╡ f2192d47-1366-4bd9-ba24-3014b0a3d09d
md"""
#### A better choice?
**(?)** If we try to estimate the ceiling function, we'd found out
```math
  m = \lceil \frac{M}{2} \rceil \implies
  \frac{M}{2} \le m \le \frac{M+1}{2}
  \implies
  M-1 \le 2m-1 \le M\,.
```
Isn't this a better choice for ``m``? ``\;2m-1`` is now even closer to ``M``.

**(R)** I think this is correct analysis. Maybe Miss O'Leary just overlooked this fact. Let's use ``m = \lceil \frac{M}{2} \rceil`` in this notebook.
"""
...
# ╔═╡ f921401d-52bf-4fd3-a2bd-9556fbd5226c
let
  alpha = 0.7
  lw = 2
  ts = range(-2,2;length=700)
...
\psi_j'(t) &= -2 \left( \frac{t - \left( t_{j-1} + \frac{h}{2} \right)}{\frac{h}{2}}  \right) \frac{2}{h} = -\frac{8}{h^2} \left(t - \left( t_{j-1} + \frac{h}{2} \right)\right) \\
\psi_j'(t_{j-1}) &= -\frac{4}{h} \\
\psi_j'(t_{j}) &= \frac{4}{h}
\end{align}
```
"""
...
function fe_quadratic_post(coefficients::Array{Float64,1})
  dim = length(coefficients)
  m = (dim+1) ÷ 2
  function u_h(t::Number)
    ## Implementation 01
    #ϕs = [ϕ(m+1,j÷2)(t) for j in 1:dim if iseven(j)]
    ## Implementation 02
    terms = [isodd(j) ? coefficients[j]*ψ(m,j÷2+1)(t) : coefficients[j]*ϕ(m+1,j÷2)(t) for j in 1:dim]
...
# ╔═╡ 2b1ea038-9e1f-4105-b2de-e7cbdb83e7ab
reduce(*, [2 3 4]), reduce(*, [2; 3; 4])

# ╔═╡ cf5e1943-6a17-4906-8051-cc8827bb1f03
let
  @benchmark m = 7 ÷ 2 + 7 % 2
end

# ╔═╡ 0e6c4903-2d07-48e6-849b-5b8417ea53f1
let
  #M = 7
  @benchmark m = ceil(Int, 7 / 2)
end
...
let
  ...
  c(t) = 0
  f(t) = π^2 * sin(π*t)
  coefficients = fe_quadratic(m, a, c, f)
  u_h = fe_quadratic_post(coefficients)
  numerical   = [u_h(t)   for t in range(0,1;length=M)[2:end-1]] 
  theoretical = [sin(π*t) for t in range(0,1;length=M)[2:end-1]]
  with_terminal() do
    println("m = $m")
    println("a(t) = 1")
