...
md"""
Next, we iterate through the directory `imData` reading and putting all the images into an array of image arrays `imgs`.
"""
...
begin
  imgs = []
  for f in readdir("imData")
    if splitext(f)[end] != ".png"
      continue
    end
    push!(imgs, load("imData/$f"))
  end
  imgs
end
...
begin
  ...
  P = 28
  img = imgs[img_id]
  jigsaw = [img[1:P, 1:P] img[1:P, P+1:2P];
            img[P+1:2P, 1:P] img[P+1:2P, P+1:2P]]
end
